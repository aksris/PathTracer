...
void Mesh::ComputeArea()
{
    ...
    for (Triangle* triangle : this->faces){
        area += Area(triangle->points[0], triangle->points[1], triangle->points[2]);
    }
}

Intersection Mesh::SampleLight(float x, float y, glm::vec3 ray_origin, IntersectionEngine* i_engine){
    return this->faces.at(rand() % this->faces.size())->SampleLight(x, y, ray_origin, i_engine);
}

Intersection Triangle::SampleLight(float x, float y, glm::vec3 ray_origin, IntersectionEngine* i_engine){
    float sx1 = glm::sqrt(x);

    float tmp_x = 1.0f - sx1;
    float tmp_y = y * sx1;
