...
Intersection SquarePlane::SampleLight(float x, float y, glm::vec3 ray_origin, IntersectionEngine* i_engine){
    glm::vec3 point(x - 0.5f, y - 0.5f, 0.0f);
    glm::vec3 pointW = glm::vec3(transform.T() * glm::vec4(point, 1.0f));
    glm::vec3 direction = glm::normalize(pointW - ray_origin);
