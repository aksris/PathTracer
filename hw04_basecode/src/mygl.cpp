...
void MyGL::SceneLoadDialog()
{
    ...
    dir_integrator = DirectLightingIntegrator();
    intersection_engine = IntersectionEngine();
    //Load new objects based on the XML file chosen.
    xml_reader.LoadSceneFromFile(file, local_path, scene, dir_integrator);
    dir_integrator.scene = &scene;
    dir_integrator.intersection_engine = &intersection_engine;
    tot_integrator.scene = &scene;
    tot_integrator.intersection_engine = &intersection_engine;
    intersection_engine.scene = &scene;
    ResizeToSceneCamera();
    update();
}
...
#define MULTITHREADED
    ...
    for(unsigned int Y = 0; Y < y_block_count; Y++)
    {
        ...
        for(unsigned int X = 0; X < x_block_count; X++)
        {
            ...
            render_threads[Y * x_block_count + X] = new RenderThread(x_start, x_end, y_start, y_end, scene.sqrt_samples, 5, &(scene.film), &(scene.camera), &(tot_integrator));
    ...
    for(unsigned int i = 0; i < scene.camera.width; i++)
    {
        ...
        {
            //358, 55
            QList<glm::vec2> sample_points = pixel_sampler.GetSamples(250, 160);
            glm::vec3 accum_color;
            for(int a = 0; a < sample_points.size(); a++)
            {
                glm::vec3 color = tot_integrator.TraceRay(scene.camera.Raycast(sample_points[a]), 0);
                accum_color += color;
            }
            scene.film.pixels[i][j] = accum_color / (float)sample_points.size();
            if(scene.film.pixels[i][j].x > 1.f) scene.film.pixels[i][j].x = 1.f;
            if(scene.film.pixels[i][j].y > 1.f) scene.film.pixels[i][j].y = 1.f;
            if(scene.film.pixels[i][j].z > 1.f) scene.film.pixels[i][j].z = 1.f;
        }
    }
#endif
